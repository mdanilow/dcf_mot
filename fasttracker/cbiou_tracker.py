...
from fasttracker.cbiou_base import TrackState, BaseTrack, BaseTracker
import fasttracker.cbiou_matching as matching
import torch 
...
class C_BIoUTracker(BaseTracker):
    def __init__(self, tracker_config,
        ...
        self.frame_count = 0
        self.det_thresh = tracker_config["det_conf_thresholds"][0]
    ...
    def update(self, det_results, features=None, debug_img=None, debug=None):
        """
        ...
        for u_track in u_tracks1:
            if self.frame_count - u_track.end_frame > self.max_time_lost:
                u_track.mark_removed()
                removed_stracks.append(u_track)
            else:
                u_track.mark_lost()
                u_track.time_since_update = self.frame_count - u_track.end_frame  # u_track.time_since_update += 1
